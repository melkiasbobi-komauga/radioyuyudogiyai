...
$limit = 10; // Jumlah agenda per halaman
$offset = ($page - 1) * $limit;

try {
    // Hitung total data untuk pagination
    // PERBAIKAN: Menggunakan parameter unik :search1 dan :search2
    $count_sql = "SELECT COUNT(*) FROM agenda WHERE nama_kegiatan LIKE :search1 OR deskripsi LIKE :search2";
    ...
    // PERBAIKAN: Menggunakan parameter unik :search1 dan :search2
    $sql = "SELECT * FROM agenda WHERE nama_kegiatan LIKE :search1 OR deskripsi LIKE :search2 ORDER BY tanggal_mulai DESC LIMIT :limit OFFSET :offset";
    ...
    die("Tidak bisa mengambil data agenda: " . $e->getMessage());
}
...
<div class="flex-1 flex flex-col bg-gray-50">
    ...
    <header class="bg-white shadow-sm p-4 flex flex-col md:flex-row justify-between items-center sticky top-0 z-10 space-y-4 md:space-y-0">
        <div class="flex items-center w-full md:w-auto">
            ...
            <h1 class="text-xl font-bold text-gray-800 tracking-tight">Kelola Agenda</h1>
        </div>
        ...
        <div class="flex flex-col md:flex-row items-center space-y-2 md:space-y-0 md:space-x-3 w-full md:w-auto">
            ...
            <a href="tambah_agenda.php"
                class="w-full md:w-auto bg-green-500 hover:bg-green-600 text-white font-medium py-2 px-4 rounded-lg transition duration-200 text-sm shadow-sm flex items-center justify-center">
                <i class="fas fa-plus fa-fw mr-2"></i>Tambah Agenda
            </a>
        </div>
    </header>
        ...
        <?php if(isset($_GET['status'])): ?>
            <div class="mb-6 p-4 rounded-lg shadow-sm flex items-start <?php echo ($_GET['status'] == 'sukses_hapus' || $_GET['status'] == 'sukses_tambah' || $_GET['status'] == 'sukses_edit') ? 'bg-green-50 text-green-700 border-l-4 border-green-500' : 'bg-red-50 text-red-700 border-l-4 border-red-500'; ?>" role="alert">
                ...
                <div>
                    ...
                    <p class="text-sm">
                        <?php 
                            if($_GET['status'] == 'sukses_tambah') echo 'Agenda baru berhasil ditambahkan.';
                            elseif($_GET['status'] == 'sukses_edit') echo 'Agenda berhasil diperbarui.';
                            elseif($_GET['status'] == 'sukses_hapus') echo 'Agenda berhasil dihapus.';
                        ...
                        <?php if (count($agendas) > 0): ?>
                            ...
                            <tr class="hover:bg-gray-50 transition duration-150">
                                <td class="px-5 py-4 bg-white text-sm">
                                    <p class="text-gray-900 font-medium text-base"><?php echo htmlspecialchars($row['nama_kegiatan']); ?></p>
                                    <?php if (!empty($row['deskripsi'])): ?>
                                        <p class="text-gray-400 text-xs mt-1 truncate w-64" title="<?php echo htmlspecialchars($row['deskripsi']); ?>">
                                            <?php echo htmlspecialchars(substr($row['deskripsi'], 0, 60)) . (strlen($row['deskripsi']) > 60 ? '...' : ''); ?>
                                        </p>
                                    <?php endif; ?>
                                </td>
                                <td class="px-5 py-4 bg-white text-sm">
                                    <div class="flex flex-col text-gray-600 text-xs">
                                        <span class="flex items-center mb-1 font-medium">
                                            <i class="far fa-calendar-alt mr-2 text-blue-400 w-4"></i>
                                            <?php echo date('d M Y', strtotime($row['tanggal_mulai'])); ?>
                                        </span>
                                        <?php if (!empty($row['waktu_mulai'])): ?>
                                        <span class="flex items-center">
                                            <i class="far fa-clock mr-2 text-orange-400 w-4"></i>
                                            <?php echo date('H:i', strtotime($row['waktu_mulai'])); ?> WIT
                                        </span>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-4 bg-white text-sm text-center">
                                    <div class="flex justify-center space-x-2">
                                        <a href="edit_agenda.php?id=<?php echo $row['id']; ?>" class="p-2 bg-yellow-50 text-yellow-600 rounded-lg hover:bg-yellow-100 transition shadow-sm" title="Edit">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <a href="proses_agenda.php?aksi=hapus&id=<?php echo $row['id']; ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus agenda ini?')" class="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition shadow-sm" title="Hapus">
                            ...
                            <tr>
                                <td colspan="4" class="px-5 py-10 border-b border-gray-200 bg-white text-sm text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center py-6">
                                        ...
                                        <p class="text-lg font-medium text-gray-600">Belum ada data agenda.</p>
                                        <?php if(!empty($search)): ?>
                                            <p class="text-gray-400 text-xs mt-1">Tidak ditemukan agenda dengan kata kunci "<?php echo htmlspecialchars($search); ?>".</p>
                                        <?php else: ?>
                                            <a href="tambah_agenda.php" class="mt-2 text-blue-500 hover:underline text-xs">Buat Agenda Baru</a>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            </tr>
